...
public class ObservacionesDAO extends ConexionBD{
    ...
    public boolean ingresar(Observaciones observaciones){
        ...
        try {
            stmt = conn.createStatement();
            String sql = "INSERT INTO OBSERVACIONES(ID_OBSERVACIONES, ID_EXPEDIENTE, OBSERVACION) VALUES("+observaciones.getIdObservacion()+", "+observaciones.getIdExpediente()+", '"+observaciones.getObservacion()+"');";
            stmt.execute(sql);
            exito = true;
            this.cerrarConexion();
        }catch (Exception e) {
