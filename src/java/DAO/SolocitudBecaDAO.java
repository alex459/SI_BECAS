...
public class SolocitudBecaDAO extends ConexionBD{
    ...
    //obtener solicitud por expediente y idusuario
     ...
     public SolicitudDeBeca consultarPorIdExpediente(int id) {
        SolicitudDeBeca temp = new SolicitudDeBeca();       
        this.abrirConexion();
        try {
            stmt = conn.createStatement();
            String sql = "SELECT ID_SOLICITUD, ID_USUARIO, ID_EXPEDIENTE, ID_OFERTA_BECA, FECHA_SOLICITUD, BENEFICIOS FROM solicitud_de_beca WHERE ID_EXPEDIENTE = " + id;
            ResultSet rs = stmt.executeQuery(sql);
            this.cerrarConexion();
